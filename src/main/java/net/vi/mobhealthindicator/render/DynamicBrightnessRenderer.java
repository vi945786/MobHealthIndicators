...
public class DynamicBrightnessRenderer {
    ...
    public static void draw(MatrixStack matrixStack, NativeImageBackedTexture texture, int light) {
        ...
        assert image != null;
        drawHeart(matrixStack.peek().getPositionMatrix(), bufferBuilder, image.getWidth() / 2f, image.getHeight(), image.getWidth(), image.getHeight(), light);
    ...
    private static void drawVertex(Matrix4f model, VertexConsumer vertices, float x, float y, float u, float v, int light) {
        vertices.vertex(model, x, y, 0).color(1F, 1F, 1F, 1F).texture(u, v).overlay(0, 10).light(light).normal(x, 0, 0).next();
    }
}
